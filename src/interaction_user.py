...
def get_aeroplanes_by_altitude(aeroplanes, altitude_range):
    ...
    try:
        ...
        return [
            ...
            if item.altitude and min_alt <= item.altitude <= max_alt
        ]
    except ValueError:
...
def sort_aeroplanes(aeroplanes):
    ...
    return sorted(
        aeroplanes, key=lambda item: item.altitude if item.altitude else 0, reverse=True
    )
...
def print_aeroplanes(aeroplanes):
    ...
    for i, a in enumerate(aeroplanes, 1):
        print(
            f"{i}. Позывной: {a.callsign} | Страна: {a.origin_country} | Высота: {a.altitude} м"
        )
